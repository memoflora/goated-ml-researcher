"""A complete, working pipeline. Handed to the agent as the starting point for a draft.

This is prompt content, not a test fixture: `agent.py` embeds it in the cached system
block so every draft starts from something that already runs end to end.

It exists because of what the live runs measured. Across sixteen iterations the agent
never once produced a valid submission, and not one failure was a modelling error — they
were argparse flags it did not declare, row orders it did not preserve, and feature
columns it computed and then dropped. It was spending its whole budget rediscovering the
contract instead of doing ML.

So the plumbing is given and the model is not. Predicting one constant is worthless as a
model and invaluable as a control: it scores what a pipeline scores having learned
nothing, and it proves the path works before anything clever is attempted. Debugging a
clever model and a broken contract at the same time is how a run dies.

Kept byte-identical in behaviour to `tests/fixtures/replay/01_constant_baseline.py`, which
`ReplayAgent` serves offline. A test runs this file in the sandbox and checks the
submission validates, so what we promise the agent is verified rather than asserted.
"""
